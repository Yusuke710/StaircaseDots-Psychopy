...
expName = 'RandomDots'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\Yusuke\\Desktop\\TlabWinter2021\\RandomDots_lastrun.py',
...
textbox.reset()
# keep track of which components have finished
survey1_3Components = [survey1, textbox, button]
...
t = 0
_timeToFirstFrame = win.getFutureFlipTime(clock="now")
survey1_3Clock.reset(-_timeToFirstFrame)  # t0 is time of first possible flip
frameN = -1

# -------Run Routine "survey1_3"-------
while continueRoutine:
    # get current time
    t = survey1_3Clock.getTime()
    tThisFlip = win.getFutureFlipTime(clock=survey1_3Clock)
    tThisFlipGlobal = win.getFutureFlipTime(clock=None)
    frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
    # update/draw components on each frame
    
    # *survey1* updates
    if survey1.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        # keep track of start time/frame for later
        survey1.frameNStart = frameN  # exact frame index
        survey1.tStart = t  # local t and not account for scr refresh
        survey1.tStartRefresh = tThisFlipGlobal  # on global time
        win.timeOnFlip(survey1, 'tStartRefresh')  # time at next scr refresh
        survey1.setAutoDraw(True)
...
# -------Ending Routine "survey1_3"-------
for thisComponent in survey1_3Components:
    if hasattr(thisComponent, "setAutoDraw"):
        thisComponent.setAutoDraw(False)
thisExp.addData('survey1.started', survey1.tStartRefresh)
thisExp.addData('survey1.stopped', survey1.tStopRefresh)
